...
LFLAGS=$(WALL) $(LIBJLIB2) -o jexpand $(LIBS)

ALL_OBJ=jexpand$(OBJ) jexpanda$(OBJ) jexpandh$(OBJ) jexpandi$(OBJ)
ALL_C=jexpand.c jexpanda.c jexpandh.c jexpandi.c
ALL_H=jexpand.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) jexpand.1$(GZ)
...
jexpand.1.gz:	jexpand.1
	-$(RM) jexpand.1.gz
	$(GZIP) jexpand.1

jexpand.1:	jexpand.man
	$(NROFF) jexpand.man > jexpand.1
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) jexpand
	$(CP) jexpand $(PRODUCTION)/jexpand
	$(CHMOD) 755 $(PRODUCTION)/jexpand
	$(CP) jexpand.1$(GZ) $(PRODMAN)/jexpand.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/jexpand.1$(GZ)

uninstall:	all
	-$(RM) $(PRODUCTION)/jexpand
	-$(RM) $(PRODMAN)/jexpand.1.gz
	-$(RM) $(PRODMAN)/jexpand.1
